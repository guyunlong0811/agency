...
class BInitController extends BaseController
{
    ...
    //选择日期
    protected function selectDate($date = null, $min = '2015-03-20', $max = '%y-%M-%d', $fmt = 'yyyy-MM-dd')
    {
        $ts = time();
        $this->vDate = is_null($date) ? time2format($ts - 86400, 2) : $date;
        $this->vDateFmt = $fmt;
        $this->vDateMin = $min;
        $this->vDateMax = $max;
    }
    ...
    //选择日期
    protected function selectDateStartEnd($start = null, $end = null, $min = '2015-03-20', $max = '%y-%M-%d', $fmt = 'yyyy-MM-dd')
    {
        $ts = time();
        $this->vDateStart = is_null($start) ? time2format($ts - 7 * 86400, 2) : $start;
        $this->vDateEnd = is_null($end) ? time2format($ts, 2) : $end;
        $this->vDateFmt = $fmt;
        $this->vDateMin = $min;
        $this->vDateMax = $max;
    }
